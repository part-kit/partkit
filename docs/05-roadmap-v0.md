...
2. **auth.session** — email + OAuth sign-in, sessions, middleware guards; wraps Better Auth behind the contract. The part everything else requires. *Shipped 1.0.0 (2026-06-11), sixth in actual build order (human-checkpoint cleared): **first OSS-wrapping part** — wraps Better Auth, declaring `better-auth` + `pg` in `npm_dependencies` (RFC 0001, contract_version 0.2). v1 is email/password + sessions + guards (`authHandler` catch-all mount, `getSession`/`requireSession`, server-side `signUp`/`signIn`/`signOut`); OAuth is an additive 1.1 (providers as seams, not adapters). Owns `auth_*` tables via Better Auth's modelName mapping, created by `partkit migrate` (migration generated from Better Auth's own schema generator). Zero registry adapters. 7 conformance tests against real Better Auth + real Postgres: password hashed (scrypt, never plaintext), no account enumeration, session invalidation. Node-runtime only (pg, not Edge). **1.1.0 (2026-06-14): OAuth / social sign-in shipped** — Google + GitHub as env-configured seams (a provider is on iff both its `*_CLIENT_ID`+`*_CLIENT_SECRET` are set), wired into Better Auth's `socialProviders`. Purely additive: the existing `authHandler` catch-all already serves `/sign-in/social` + `/callback/:provider`, and `auth_account` already had the OAuth columns, so **no new export, no migration**. 9 conformance tests (the 2 new ones assert a configured provider returns the real Google consent URL with the client id, and an unconfigured provider is rejected — both offline). Focused adversarial review: clean ("ship it"). Email verification + password reset (composing on `email.transactional`) remain the roadmap.*
3. **billing.subscription** — checkout, webhooks, plan state, cancel/upgrade. Hardest conformance (replay, ordering, races); the flagship attestation. v1 verifies its own inbound webhooks; may compose on `webhooks.ingest` from v2. *Shipped 1.0.0 (2026-06-12), tenth and last in actual build order — completes the saas core ten. Stripe adapter attested (`stripe@^22`, API pinned `2026-05-27.dahlia`); per-adapter `npm_dependencies` so a future Paddle adapter never drags in Stripe. Hosted Checkout + a webhook-derived subscription mirror on the `SqlExecutor` seam (`billing_subscriptions` + an append-only `billing_events` idempotency ledger); cancel/reactivate/change-plan; `entitled = status ∈ {active, trialing}`. **State derives solely from verified webhooks** — checkout writes no row, the success redirect grants nothing. Inbound verification is the part's own raw-HMAC (the `webhooks.ingest` mechanics, cross-checked against Stripe's own signer), so it is SDK/network-free; `email.transactional` (dunning) is deferred. 18 conformance tests against the **real Stripe test API** + real Postgres (item-level `current_period_end`, replay/idempotency, injection, secret redaction, append-only ledger). admin reads are read-only (RFC 0004). **1.1.0 (2026-06-14): the Paddle adapter shipped — the attested vendor-flip is now REAL** (`partkit upgrade billing.subscription --adapter=paddle`). Zero-dependency (raw `fetch` + `node:crypto`, no Paddle SDK): writes via POST /transactions (hosted checkout) + /customers + /subscriptions cancel/PATCH; verifier is the same raw-HMAC pattern over Paddle's wire format (`Paddle-Signature: ts=..;h1=..`, signed bytes `ts:rawBody`, `pdl_ntfset_` key). The conformance suite became adapter-neutral (one suite, per-vendor profile keyed on `adapter.name`, the email.transactional pattern) + a paddle-only offline-writes block against a fake Paddle server; stripe 28 green vs real Stripe + PG, paddle 33 green. A 5-lens adversarial review fixed a real HIGH (raw `fetch` had no timeout → added an AbortController deadline) + a defensive HIGH (facade now refuses to mint a mirror row with an empty vendor subscription id). The `stripe_*` column/field names stay (additive minor; a 2.0 may rename to vendor-neutral). Dunning email remains the roadmap.*
...
|---|---|
| Sign in (email + OAuth) | `auth.session` ✅ (email/password + Google/GitHub OAuth, 1.1.0) |
...
**Beyond the core — vendor-flip twins (2026-06-14):** **`billing.subscription` 1.1.0**
added the **paddle** adapter (the real attested vendor-flip on the hero part), and
**`sms.transactional` 1.0.0** shipped as the SMS twin of `email.transactional`:
**twilio** (one Basic-auth REST call — the easy/pricey vendor an agent reaches for)
vs **amazon-sns** (SigV4-signed `Publish` — the fiddly/cheaper one, reusing the SES
signer with `service=sns`). Both zero-dependency, both a one-command flip
(`partkit upgrade … --adapter=…`). Registry: 16 parts. These make the
vendor-economics homepage story (email + SMS + billing) two worked examples, not
one. SMS is stateless (no DB); 11→15 conformance tests/adapter, a 4-lens review
(code clean; conformance hardened: SNS signature recomputed from the recorded
request, exact-id round-trip, the Twilio MG-SID/`TWILIO_FROM` branches, network
retry).
