...
- ✅ `billing.subscription` 1.0.0 — **the flagship**. Stripe adapter attested (`stripe@^22`, API pinned `2026-05-27.dahlia`; per-adapter `npm_dependencies`, auto-isolated). Hosted Checkout + a **webhook-derived** subscription mirror on the `SqlExecutor` seam; cancel/reactivate/change-plan; `entitled = status ∈ {active, trialing}`. **State derives solely from verified webhooks** (checkout writes no row). Inbound verification is the part's own **raw-HMAC** (the webhooks.ingest mechanics, cross-checked against Stripe's own signer — SDK/network-free), so v1 takes no `webhooks.ingest` edge; `email.transactional` (dunning) deferred. Idempotency by vendor event id over an append-only `billing_events` ledger; no card data stored; admin reads read-only (RFC 0004). 18 conformance tests against the **real Stripe test API** + real Postgres. **Completes the `saas` core ten.**

**Wave 1 is complete — the `saas` core ten are all shipped and attested.** The acme demo (docs/05 §2b) is now assemblable end-to-end; the next finish-line work is the §8 "done well" demonstration. New part work proceeds to Wave 2.
