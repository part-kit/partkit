...
## SES adapter (added in 1.1.0)

Amazon SES via the **SES v2 SendEmail** REST API (`POST
email.{region}.amazonaws.com/v2/email/outbound-emails`), signed with **SigV4 by
hand** (`src/internal/sigv4.ts`, header-authorization variant) — **zero npm
dependencies, no `aws-sdk`**. This is the whole point of the adapter: the SES
integration that AI agents avoid (signing, the v2 wire format, IAM) lives here
once, attested, so the app's choice is one line and the resend→SES swap is one
commit (the planner's cheapest pick becomes real). Conformance proves the wire
behavior against the protocol-faithful fake (auth header structure, body shape,
retry/typed-error/redaction) **and** anchors the SigV4 HMAC chain to AWS's own
documented signing-key known-answer vector — so a wrong signature can't pass.
The one irreducible cost is account-side and human (verify identity + DKIM,
request production access out of the sandbox); seams.md §1b makes it a checklist.
Attested as a first-class adapter — the protocol-faithful fake + the AWS
known-answer vector make it honest without a live AWS account, exactly as
`storage.upload` anchors its SigV4 to AWS-CLI vectors.

## Roadmap

- Delivery events via `webhooks.ingest` composition —
  `events: ["email.delivered", "email.bounced", "email.complained"]`.
- An optional gated real-SES sandbox send in conformance once a test AWS account
  is wired into the verification CI (offline vectors already attest the signing).
