...
class BillingImpl implements Billing {
  ...
  /** The request header THIS adapter's webhook signature arrives in. */
  ...
  constructor(
    ...
    this.signatureHeaderName = adapter.webhookSignatureHeader;
  }
  ...
  private async applyMirror(
    ...
    // A malformed (but signature-verified) event with no subscription id must
    // never mint a mirror row — an empty id would collide on the primary key.
    if (r.vendorSubscriptionId === "") return null; // recorded in the ledger, no mirror
 ...
 *   200 — verified and (idempotently) applied; safe for the vendor to stop retrying.
 ...
 */
async function routeIngest(b: BillingImpl, request: Request): Promise<Response> {
  const rawBody = new Uint8Array(await request.arrayBuffer());
  const signatureHeader = request.headers.get(b.signatureHeaderName) ?? "";
  try {
...
export function makeWebhookHandler(
  ...
  const impl = new BillingImpl(db, adapter);
  return (request: Request) => routeIngest(impl, request);
}
