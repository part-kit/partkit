...
contract-stable interface, wrapping Better Auth over part-owned tables. v1
scope is **email/password + sessions + guards**.
...
Invariants 1 and 7 run DB-free; 2–6 run against real Better Auth + real
...
- `1.1` (minor, additive): OAuth / social sign-in — providers configured as
  seams (client id/secret env per provider), not registry adapters; the same
  `auth_account` table already carries provider columns.
