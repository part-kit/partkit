...
const GOOGLE_CLIENT_SECRET = "test-google-client-secret-value";
...
describe.skipIf(PG_URL === undefined || PG_URL === "")(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      process.env["GOOGLE_CLIENT_SECRET"] = GOOGLE_CLIENT_SECRET; // github intentionally left unset
    });
    ...
    it("invariant 8: a configured social provider (google) returns a provider redirect URL carrying the client id", async () => {
      const { authHandler } = await import("../src/index");
      const req = new Request("http://localhost:3000/api/auth/sign-in/social", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ provider: "google", callbackURL: "http://localhost:3000/dashboard" }),
      });
      const res = await authHandler(req);
      expect(res.status).toBeLessThan(400);
      const data = (await res.json()) as { url?: string };
      // Better Auth builds the consent URL locally (no network) — it points at
      // Google and carries our client id, proving the provider is wired.
      expect(data.url ?? "").toContain("accounts.google.com");
      expect(data.url ?? "").toContain(GOOGLE_CLIENT_ID);
    });

    it("invariant 8: an UNCONFIGURED social provider (github) is rejected, not silently attempted", async () => {
      const { authHandler } = await import("../src/index");
      const req = new Request("http://localhost:3000/api/auth/sign-in/social", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ provider: "github", callbackURL: "http://localhost:3000/dashboard" }),
      });
      const res = await authHandler(req);
