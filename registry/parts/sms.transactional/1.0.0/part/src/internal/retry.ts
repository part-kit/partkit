 ...
 * adapter marked retryable (429, 5xx, network) are retried; permanent failures
 * surface immediately (contract invariant 4).
 */
export async function withRetry<T>(fn: () => Promise<T>): Promise<T> {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      lastError = e;
      const retryable = e instanceof SmsError && e.retryable;
