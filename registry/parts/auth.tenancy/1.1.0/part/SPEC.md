...
**1.1.0 (additive minor):** the contract gains `data_ownership.reads` (RFC 0004),
declaring the admin read surface for `auth_tenant_organization` (delete →
`deleteOrganization`) and `auth_tenant_membership` (update → `setRole`, delete →
`removeMember`); `user_id` is an opaque cross-part id (`references_capability:
auth.session`), no secret columns are exposed. So `admin.crud` can administer
these tables — reading only declared columns, writing only through the mutators
above, so the never-ownerless / last-owner guards still hold. No interface,
schema, or seam change (`migrations/1.0.0-1.1.0/seam-changes.md`).
